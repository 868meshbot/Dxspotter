# DXSpotter — post-build script: produces named app-only and merged binaries
# Copyright 2026 DXSpotter Contributors
# SPDX-License-Identifier: GPL-3.0-or-later
#
# Output (in firmware/):
#   dxspotter-<device>-<version>.bin           — app-only (flash at 0x10000)
#   dxspotter-<device>-<version>-merged.bin    — bootloader+table+app (flash at 0x0)
#
...
# Example: 0.1.0-beta.3+47.20260609.abc1234
...
def _read_version(project_dir):
    ...
    try:
        m = re.search(r'DXS_VERSION\s+"([^"]+)"', open(path).read())
...
def build_firmware_artifacts(source, target, env):
    ...
    stem     = "dxspotter-%s-%s" % (device, version)
